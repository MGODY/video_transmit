...
VideoDevice::VideoDevice(QString device_name):fd(-1),dev_name(device_name),buffers(NULL),n_buffers(0),index(-1) {}
...
int VideoDevice::open_device(){
    ...
    if(-1==fd){
    //emit display_error(tr("camera open error: %1 ").arg(QString(strerror(errno))));
	return fd;
    }
...
int VideoDevice::init_device(){
    ...
    if(-1 == ioctl(fd, VIDIOC_QUERYCAP, &cap))
    {
        if(EINVAL == errno)
        {
            ...
            cout<<"the camera is no V4l2 device"<<endl;
        }
        else
        {
    ...
    if(!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE))
    {
        ...
        cout<<"the camera is no video capture device"<<endl;
        return -1;
    }
    ...
    if(!(cap.capabilities & V4L2_CAP_STREAMING))
    {
        ...
        cout<<"the camera does not support streaming i/o"<<endl;
        return -1;
    }
  ...
  if( -1 == ioctl(fd,VIDIOC_S_PARM,&parm) )
    //emit display_error(tr("set rate error").arg(QString(strerror(errno))));
    cout<<"set rate error"<<endl;
  cout<<"rate: "<<parm.parm.capture.timeperframe.denominator<<endl;
...
int VideoDevice::init_mmap(){
    ...
    if(-1 == ioctl(fd, VIDIOC_REQBUFS, &req))
    {
        if(EINVAL == errno)
        {
            ...
            cout<<"the camera does not support memory mapping"<<endl;
            return -1;
        }
        else
        {
            //emit display_error(tr("VIDIOC_REQBUFS %1").arg(QString(strerror(errno))));
            cout<<"set VIDIOC_REQBUFS error"<<endl;
            return -1;
        }
    }
